...
build: ## build images
		@echo "Building image gui"
		@docker buildx build --progress auto --load -f docker/Dockerfile --target gui -t edgd1er/py-kms .
		@echo "Building image minimal"
		@docker buildx build --progress auto --load -f docker/Dockerfile --target minimal -t edgd1er/py-kms-minimal .
